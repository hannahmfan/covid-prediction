...
class DelphiModel:
    ...
    # test_per: number of training data points to cut off for testing purposes
    # pred_per: number of days into the future from last data point to predict
    ...
    def predict_one_county(county, last_train_date, num_pred_steps, submission=None, use_sample=True, drop_negative=True, verbose=True):
        ...
        last_date = datetime.fromisoformat(county_data[-1]["date"])
        train_date = datetime.fromisoformat(last_train_date)
        test_per = 

        if test_per > 0: df = [:-test_per]
        ...
        try: 
            ...
            if len(df) == 0:
                if verbose: print("No data found for county", str(county), ":", row["COUNTY"])
                return submission
            else:
                cum_deaths = 0
