...
def train_neural_nets(helper, erf, erf_pred_dates, erf_train_cutoff, target_len):
    c_map = helper.map_counties_to_deaths(fips_list)
    ...
    large_NN = ResidualNN(
        ...
        STEP = 1, TRAIN_SPLIT = int(0.8 * len(training_data_l)), BATCH_SIZE = 64, BUFFER_SIZE = 10000
    )
    
    small_NN = ResidualNN(
        training_data = training_data_s.sample(frac=1),
        history_len = 10, target_len = target_len,
        STEP = 1, TRAIN_SPLIT = int(0.8 * len(training_data_l)), BATCH_SIZE = 256, BUFFER_SIZE = 10000
    )
...
class NN_Residuals_Model:
    ...
    def predict_all_counties(final_pred_dates, erf_pred_dates, erf_training_cutoff, final_training_cutoff, verbose=True):
        ...
        large_NN, small_NN = train_neural_nets(helper, erf_init, erf_pred_dates, erf_training_cutoff, len(final_pred_dates))
        ...
        for idx, row in fips_list.iterrows():
            ...
            if cum_deaths > 200:
                pred = large_NN.predict_one_county(
                    county, erf_init, erf_final, helper, datetime.fromisoformat(erf_training_cutoff), 
        ...
        submission["10"] = submission["10"].apply(lambda x: x if float(x) >= 1 else 0)
        submission["20"] = submission["20"].apply(lambda x: x if float(x) >= 1 else 0)
        submission["30"] = submission["30"].apply(lambda x: x if float(x) >= 1 else 0)
        submission["40"] = submission["40"].apply(lambda x: x if float(x) >= 1 else 0)
        submission["50"] = submission["50"].apply(lambda x: x if float(x) >= 1 else 0)
        submission["60"] = submission["60"].apply(lambda x: x if float(x) >= 1 else 0)
        submission["70"] = submission["70"].apply(lambda x: x if float(x) >= 1 else 0)
        submission["80"] = submission["80"].apply(lambda x: x if float(x) >= 1 else 0)
        submission["90"] = submission["90"].apply(lambda x: x if float(x) >= 1 else 0)
    ...
    def predict_one_county(county, final_pred_dates, erf_pred_dates, erf_training_cutoff, final_training_cutoff):
        ...
        large_NN, small_NN = train_neural_nets(erf_init, erf_pred_dates, erf_training_cutoff, len(final_pred_dates))
